  ...
  float scale)
{
  ...
  if (numel <= 768 && !scaleGradByFreq) {
    hipLaunchKernelV2(HIP_KERNEL_NAME(cunn_LookupTable_accGradParametersKernelByFeature), dim3(DIVUP(stride,4)), dim3(128), 0, stream, 
  ...
  dim3 block(32, 4);
  hipLaunchKernelV2(HIP_KERNEL_NAME(cunn_LookupTable_accGradParametersKernel), dim3(grid), dim3(block), 0, stream, 
    sorted_data,
    indices_data,
    THCudaTensor_data(state, gradOutput),
    THCudaTensor_data(state, gradWeight),
