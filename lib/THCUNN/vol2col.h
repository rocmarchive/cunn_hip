...
void vol2col(hipStream_t stream, const Dtype* data_vol, const int channels,
  ...
  // Launch
  wstLaunchKernel(HIP_KERNEL_NAME(vol2col_kernel), dim3(GET_BLOCKS(num_kernels)), dim3(CUDA_NUM_THREADS), 0, stream, 
...
void col2vol(hipStream_t stream, const Dtype* data_col, const int channels,
  ...
  wstLaunchKernel(HIP_KERNEL_NAME(vol2im_kernel), dim3(GET_BLOCKS(num_kernels)), dim3(CUDA_NUM_THREADS), 0, stream, 
