...
void vol2col(hipStream_t stream, const Dtype* data_vol, const int channels,
  ...
  // Launch
  hipLaunchKernel((vol2col_kernel), dim3(GET_BLOCKS(num_kernels)), dim3(CUDA_NUM_THREADS), 0, stream, 
...
void col2vol(hipStream_t stream, const Dtype* data_col, const int channels,
  ...
  hipLaunchKernel((vol2im_kernel), dim3(GET_BLOCKS(num_kernels)), dim3(CUDA_NUM_THREADS), 0, stream, 
