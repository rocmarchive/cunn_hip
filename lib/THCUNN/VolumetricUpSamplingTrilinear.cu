...
__global__ void caffe_gpu_interp2_kernel(const int n,
  ...
  if (index < n) {
    ...
    for (int n = 0; n < batchsize ; n++){
        for (int c = 0; c < channels; ++c) {
        const Acctype val = t0lambda * (h0lambda * (w0lambda * (Acctype)data1[n][c][t1][h1][w1] 
                                                  + w1lambda * (Acctype)data1[n][c][t1][h1][w1+w1p])
                                      + h1lambda * (w0lambda * (Acctype)data1[n][c][t1][h1+h1p][w1]
                                                  + w1lambda * (Acctype)data1[n][c][t1][h1+h1p][w1+w1p]))
                          + t1lambda * (h0lambda * (w0lambda * (Acctype)data1[n][c][t1+t1p][h1][w1] 
                                                  + w1lambda * (Acctype)data1[n][c][t1+t1p][h1][w1+w1p])
                                      + h1lambda * (w0lambda * (Acctype)data1[n][c][t1+t1p][h1+h1p][w1]
                                                  + w1lambda * (Acctype)data1[n][c][t1+t1p][h1+h1p][w1+w1p]));
...
__global__ void caffe_gpu_interp2_kernel_backward(const int n,
  ...
  if (index < n) {
    ...
    if (depth1 == depth2 && height1 == height2 && width1 == width2) {
      ...
      for (int n = 0; n < batchsize ; n++){
        for (int c = 0; c < channels; ++c) {
          const Acctype val = (Acctype)data1[n][c][t2][h2][w2] + (Acctype)data2[n][c][t1][h1][w1];
          data1[n][c][t2][h2][w2] = ScalarConvert<Acctype, Dtype>::to(val);
        }
      }
      return;
    }
