          ...
          THCudaTensor *total_weight)
{
  ...
  hipLaunchKernel((cunn_SpatialClassNLLCriterion_updateOutput_kernel), dim3(total_blocks), dim3(CUDA_NUM_THREADS), 0, THCState_getCurrentStream(state), 
      output_data,
      total_weight_data,
      input_data,
      target_data,
          ...
          THCudaTensor *total_weight)
{
  ...
  hipLaunchKernel((cunn_SpatialClassNLLCriterion_updateGradInput_kernel), dim3(total_blocks), dim3(CUDA_NUM_THREADS), 0, THCState_getCurrentStream(state), 
      gradInput_data,
      target_data,
      weights_data,
      total_weight_data,
