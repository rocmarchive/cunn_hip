           ...
           THCTensor *total_weight)
{
  ...
  hipLaunchKernelGGL((cunn_SpatialClassNLLCriterion_updateGradInput_kernel<real>), dim3(total_blocks), dim3(CUDA_NUM_THREADS), 0, THCState_getCurrentStream(state),
      gradInput_data,
      target_data,
      weights_data,
      total_weight_data,
