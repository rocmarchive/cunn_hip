           ...
           bool count_include_pad)
{
  ...
  while (totalZ > 0) {
    ...
    switch (kW)
      ...
      default:
        hipLaunchKernelGGL((cuda_VolumetricAveragePooling_updateOutput<real, accreal>), dim3(grid), dim3(block), 0, 0, 
                                                                    cudaInput,
                                                                    cudaOutput,
                                                                    kT, kH, kW,
                                                                    dT, dH, dW,
           ...
           bool count_include_pad)
{
  ...
  if (dT == 1 && dH == 1 && dW == 1 && padT == 0 && padH == 0 && padW == 0)
  {
    ...
    while (totalZ > 0) {
      dim3 grid(THCCeilDiv(inputWidth, static_cast<int>(block.x)),
                ...
                totalZ > 65535 ? 65535 : totalZ);
      hipLaunchKernelGGL((cuda_VolumetricAveragePooling_updateGradInput_Stride1<real, accreal>), dim3(grid), dim3(block), 0, 0, 
  ...
  else
  {
    ...
    while (totalZ > 0) {
      ...
      if (kernelsOverlap)
        {
          hipLaunchKernelGGL((cuda_VolumetricAveragePooling_updateGradInput_atomicAdd<real, accreal>), dim3(grid), dim3(block), 0, 0,
            cudaGradOutput, cudaGradInput, kT, kH, kW, dT, dH, dW,
            padT, padH, padW, count_include_pad, offsetZ);
        }
      else
        {
          hipLaunchKernelGGL((cuda_VolumetricAveragePooling_updateGradInput<real, accreal>), dim3(grid), dim3(block), 0, 0,
             cudaGradOutput, cudaGradInput, kT, kH, kW, dT, dH, dW,
             padT, padH, padW, count_include_pad, offsetZ);
        }
      THCudaCheck(hipGetLastError());
