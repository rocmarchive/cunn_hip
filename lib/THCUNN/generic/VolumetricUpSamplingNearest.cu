           ...
           int scale_factor)
{
  ...
  // kernel:
  hipLaunchKernelGGL((vupscale), blocks, threads, 0, THCState_getCurrentStream(state), input_data, output_data, no_elements, scale_factor, d1, d2, d3, d4);
           ...
           int scale_factor)
{
  ...
  // kernel:
  hipLaunchKernelGGL((vdownscale<real ,accreal>), blocks, threads, 0, THCState_getCurrentStream(state), gradInput_data, gradOutput_data, no_elements,
