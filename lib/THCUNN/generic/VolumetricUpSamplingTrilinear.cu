           ...
           int outputWidth)
{
  ...
  hipLaunchKernelGGL((caffe_gpu_interp2_kernel<real, accreal>), THCCeilDiv(num_kernels, num_threads), num_threads,
           ...
           int outputWidth)
{
  ...
  const int num_threads =
    THCState_getCurrentDeviceProperties(state)->maxThreadsPerBlock;
  hipStream_t stream = THCState_getCurrentStream(state);
  hipLaunchKernelGGL((caffe_gpu_interp2_kernel_backward<real ,accreal>), THCCeilDiv(num_kernels, num_threads),
