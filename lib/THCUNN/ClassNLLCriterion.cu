...
void THNN_CudaClassNLLCriterion_updateOutput(THCState *state, THCudaTensor *input, THCudaLongTensor *target, THCudaTensor *output, bool sizeAverage, THCudaTensor *weights, THCudaTensor *total_weight) {
  ...
  if (THCudaTensor_nDimension(state, input) == 1) {
    hipLaunchKernelV2(HIP_KERNEL_NAME(cunn_ClassNLLCriterion_updateOutput_kernel1), dim3(1), dim3(1), 0, THCState_getCurrentStream(state), 
        output_data,
        total_weight_data,
        input_data,
        target_data,
    ...
    hipLaunchKernelV2(HIP_KERNEL_NAME(cunn_ClassNLLCriterion_updateOutput_kernel), dim3(1), dim3(NTHREADS), 0, THCState_getCurrentStream(state), 
        output_data,
        total_weight_data,
        input_data,
        target_data,
...
void THNN_CudaClassNLLCriterion_updateGradInput(THCState *state, THCudaTensor *input, THCudaLongTensor *target, THCudaTensor *gradInput, bool sizeAverage, THCudaTensor *weights, THCudaTensor *total_weight) {
  ...
  if (THCudaTensor_nDimension(state, input) == 1) {
    hipLaunchKernelV2(HIP_KERNEL_NAME(cunn_ClassNLLCriterion_updateGradInput_kernel1), dim3(1), dim3(1), 0, THCState_getCurrentStream(state), 
        gradInput_data,
        weights_data,
        target_data,
        total_weight_data,
    ...
    hipLaunchKernelV2(HIP_KERNEL_NAME(cunn_ClassNLLCriterion_updateGradInput_kernel), dim3(1), dim3(NTHREADS), 0, THCState_getCurrentStream(state), 
        gradInput_data,
        target_data,
        weights_data,
        total_weight_data,
