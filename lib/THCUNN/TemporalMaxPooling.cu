...
void THNN_CudaTemporalMaxPooling_updateOutput(
  ...
  hipLaunchKernel((cunn_TemporalMaxPooling_updateOutputKernel), dim3(blocks), dim3(threads), 0, THCState_getCurrentStream(state) , 
...
void THNN_CudaTemporalMaxPooling_updateGradInput(
  ...
  dim3 threads(nthreads);
  if (kW <= dW) {
    hipLaunchKernel((cunn_TemporalMaxPooling_updateGradInputKernel), dim3(blocks), dim3(threads), 0, THCState_getCurrentStream(state) , 
        gradInput_data, gradOutput_data, indices_data, input_w, input_n, output_w, kW, dW);
  } else {
    hipLaunchKernel((cunn_TemporalMaxPooling_updateGradInputKernelAtomic), dim3(blocks), dim3(threads), 0, THCState_getCurrentStream(state) , 
        gradInput_data, gradOutput_data, indices_data, input_w, input_n, output_w, kW, dW);
  }
  THCudaCheck(hipGetLastError());
