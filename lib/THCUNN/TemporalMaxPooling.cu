...
__global__ void cunn_TemporalMaxPooling_updateOutputKernel(hipLaunchParm lp, float *input, float *output, float *indices, int input_w, int input_n, int output_w, int kW, int dW) {
  ...
  float *output_data = output + hipBlockIdx_x * output_w * input_n + (
      hipThreadIdx_x + hipBlockIdx_y * TEMPORAL_MAX_POOLING_THREADS) * input_n;
  float *indices_data = indices + hipBlockIdx_x * output_w * input_n + (
      hipThreadIdx_x + hipBlockIdx_y * TEMPORAL_MAX_POOLING_THREADS) * input_n;
...
__global__ void cunn_TemporalMaxPooling_updateGradInputKernel(hipLaunchParm lp, float *gradInput, float *gradOutput, float *indices, int input_w, int input_n, int output_w, int kW, int dW) {
  ...
  float *gradInput_data = gradInput + hipBlockIdx_x * input_w * input_n + (
      hipThreadIdx_x + hipBlockIdx_y * TEMPORAL_MAX_POOLING_THREADS) * input_n * dW;
  float *gradOutput_data = gradOutput + hipBlockIdx_x * output_w * input_n + (
      hipThreadIdx_x + hipBlockIdx_y * TEMPORAL_MAX_POOLING_THREADS) * input_n;
  float *indices_data = indices + hipBlockIdx_x * output_w * input_n + (
      hipThreadIdx_x + hipBlockIdx_y * TEMPORAL_MAX_POOLING_THREADS) * input_n;
...
void THNN_CudaTemporalMaxPooling_updateOutput(
  ...
  stub_hipLaunchKernel(HIP_KERNEL_NAME(cunn_TemporalMaxPooling_updateOutputKernel), dim3(blocks), dim3(threads), 0, THCState_getCurrentStream(state) , 
...
void THNN_CudaTemporalMaxPooling_updateGradInput(
  ...
  dim3 threads(nthreads);
  if (kW <= dW) {
    stub_hipLaunchKernel(HIP_KERNEL_NAME(cunn_TemporalMaxPooling_updateGradInputKernel), dim3(blocks), dim3(threads), 0, THCState_getCurrentStream(state) , 
        gradInput_data, gradOutput_data, indices_data, input_w, input_n, output_w, kW, dW);
  } else {
    stub_hipLaunchKernel(HIP_KERNEL_NAME(cunn_TemporalMaxPooling_updateGradInputKernelAtomic), dim3(blocks), dim3(threads), 0, THCState_getCurrentStream(state) , 
        gradInput_data, gradOutput_data, indices_data, input_w, input_n, output_w, kW, dW);
  }
  THCudaCheck(hipGetLastError());
