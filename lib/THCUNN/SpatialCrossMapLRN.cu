    ...
    THCudaTensor* scale, int local_size, float alpha, float beta, float k)
{
  ...
  wstLaunchKernel(HIP_KERNEL_NAME(LRNFillScale), dim3(GET_BLOCKS(n_threads)), dim3(CUDA_NUM_THREADS), 0, THCState_getCurrentStream(state), 
  ...
  wstLaunchKernel(HIP_KERNEL_NAME(LRNComputeOutput), dim3(GET_BLOCKS(n_threads)), dim3(CUDA_NUM_THREADS), 0, THCState_getCurrentStream(state), 
    ...
    int local_size, float alpha, float beta, float k)
{
  ...
  int n_threads = batchSize * imsize_h * imsize_w;
  wstLaunchKernel(HIP_KERNEL_NAME(LRNComputeDiff), dim3(GET_BLOCKS(n_threads)), dim3(CUDA_NUM_THREADS), 0, THCState_getCurrentStream(state), 
